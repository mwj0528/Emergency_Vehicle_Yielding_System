...
AMBULANCE_SAFE_DISTANCE = 20.0  # 구급차와 다른 차량 사이의 안전 거리 (미터) - 15.0에서 20.0으로 증가
...
REGULAR_CAR_SAFE_DISTANCE = 10.0  # 일반 차량 간 안전 거리 (미터) - 8.0에서 10.0으로 증가

# 양보 동작 설정
YIELD_DISTANCE = 20.0  # 구급차 감지 시 양보를 시작할 거리 (미터) - 50.0에서 30.0으로 감소
YIELD_SPEED_REDUCTION = 70  # 양보 시 속도 감소 비율 (%) - 50에서 70으로 증가
YIELD_DURATION = 7.0  # 양보 상태 유지 시간 (초) - 5.0에서 7.0으로 증가
DEFAULT_CAR_SPEED = -10  # 일반 차량의 기본 속도 (제한 속도 대비) - -20에서 -10으로 변경
INTERSECTION_RADIUS = 60.0  # 교차로 영향 반경 (미터) - 50.0에서 60.0으로 증가
INTERSECTION_APPROACH_DISTANCE = 40.0  # 교차로 접근 거리 (미터) - 30.0에서 40.0으로 증가
INTERSECTION_EXIT_DISTANCE = 30.0  # 교차로 이탈 거리 (미터) - 20.0에서 30.0으로 증가
INTERSECTION_SPEED_REDUCTION = 50  # 교차로 통과 시 속도 감소 비율 (%) - 30에서 50으로 증가
INTERSECTION_STOP_DISTANCE = 20.0  # 교차로 진입 전 정지 거리 (미터) - 15.0에서 20.0으로 증가
STOPPED_VEHICLE_SAFE_DISTANCE = 15.0  # 정지된 차량과의 안전 거리 (미터) - 12.0에서 15.0으로 증가
...
DETECTION_DISTANCE_THRESHOLD = 80.0 # 구급차 후방 탐지 활성화 거리 (미터) - 120.0에서 80.0으로 감소
LANE_CHECK_DISTANCE = 30.0   # 차선 비교를 시작할 거리 (DETECTION_DISTANCE_THRESHOLD보다 약간 작게) - 50.0에서 30.0으로 감소
LATERAL_THRESHOLD = 3.5      # 같은 차선으로 간주할 측면 거리 임계값 (미터)
...
def game_loop(args):
    ...
    # 일반 차량 평균 속도 계산을 위한 변수 추가
    total_speed_sum = 0.0  # 모든 시간대의 평균 속도 합
    speed_measurement_count = 0  # 속도 측정 횟수
    ...
    try:
        ...
        while True:
            ...
            if args.use_yolo_yielding and yolo_model and camera_blueprint:
                ...
                for car_id, car_data in regular_cars.items():
                    ...
                    car_loc = car.get_location()
                    ambulance_loc = ambulance.get_location()
                    distance_to_ambulance = car_loc.distance(ambulance_loc)
                    
                    # 구급차와 같은 차선에 있는지 확인
                    is_same_lane = is_in_same_lane(car, ambulance, world_map)
                    
                    # 구급차와 같은 차선에 있거나 10m 이내에 있는 경우
                    if is_same_lane or distance_to_ambulance < 10.0:
                        print(f"[{current_sim_time:.2f}s] Car {car_id} in same lane or near ambulance")
                        # YOLO 양보 시스템 사용 시에만 신호 무시
                        if args.use_yolo_yielding:
                            ...
                            # YOLO 양보 시스템을 사용하지 않는 경우 항상 신호 준수
                            traffic_manager.ignore_lights_percentage(car, 0)  # 신호등 준수
                            traffic_manager.ignore_signs_percentage(car, 0)   # 표지판 준수
                            # 구급차가 지나갈 때까지 현재 속도 유지
                            traffic_manager.vehicle_percentage_speed_difference(car, 0)
                    else:
                        # 구급차와 다른 차선이고 멀리 있는 경우
                        # print(f"[{current_sim_time:.2f}s] Car {car_id} not in same lane and far from ambulance")
                        traffic_manager.ignore_lights_percentage(car, 0)  # 신호등 준수
                        traffic_manager.ignore_signs_percentage(car, 0)   # 표지판 준수
                    ...
                    # 카메라 활성화/비활성화 로직
                    if is_same_lane and car_data['camera'] is None:
                    ...
                    elif not is_same_lane and car_data['camera'] is not None:
                        ...
                        try:
                            ...
                            if ambulance_detected_by_yolo:
                                if current_sim_time - car_data['last_yield_time'] > yield_cooldown:
                                    print(f"[{current_sim_time:.2f}s] Car {car_id} detected ambulance by YOLO")
            ...
            if ambulance and ambulance.is_alive:
                ...
                # 일반 차량들의 현재 평균 속도 계산 및 누적
                current_speeds = []
                for car_id, car_data in regular_cars.items():
                    if car_data['actor'] and car_data['actor'].is_alive:
                        speed = car_data['actor'].get_velocity().length() * 3.6  # m/s to km/h
                        current_speeds.append(speed)
                
                if current_speeds:  # 활성 차량이 있는 경우에만
                    current_avg_speed = sum(current_speeds) / len(current_speeds)
                    total_speed_sum += current_avg_speed
                    speed_measurement_count += 1
                    
                    # 10초마다 현재 평균 속도 출력 (디버깅용)
                    if frame_count % 200 == 0:
                        print(f"[{current_sim_time:.2f}s] Current average speed of regular cars: {current_avg_speed:.2f} km/h")
        ...
        # 실험 결과 저장
        # 전체 시간에 대한 일반 차량들의 평균 속도 계산
        overall_avg_speed = total_speed_sum / speed_measurement_count if speed_measurement_count > 0 else 0
        print(f"\nRegular cars speed statistics:")
        print(f"Total speed measurements: {speed_measurement_count}")
        print(f"Overall average speed: {overall_avg_speed:.2f} km/h")
        ...
        results = {
            ...
            "regular_cars_average_speed": {
                "value": round(overall_avg_speed, 2),
                "unit": "km/h"
            },
            "regular_cars_stats": {
                "total_cars": len(regular_cars),
                "active_cars": len([car_id for car_id, car_data in regular_cars.items() if car_data['actor'] and car_data['actor'].is_alive]),
                "speed_measurements": speed_measurement_count,
                "speed_distribution": {
                    "min": round(min(current_speeds), 2) if current_speeds else 0,
                    "max": round(max(current_speeds), 2) if current_speeds else 0,
                    "median": round(sorted(current_speeds)[len(current_speeds)//2], 2) if current_speeds else 0
                }
            }
        }
        ...
        total_simulation_ran_time = world.get_snapshot().timestamp.elapsed_seconds - simulation_start_time if simulation_start_time > 0 and world is not None else 0
...
# 교차로
# python simulation_3900_traffic.py --map Town10HD --use_yolo_yielding --num_regular_cars 50 --duration 0 --iterations 1 --seed 20

# python simulation_3900_traffic.py --map Town10HD --num_regular_cars 50 --duration 0 --iterations 1 --seed 20
